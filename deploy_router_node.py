...
_STRATEGY_BASES: dict[str, str] = {
    ...
    "contrarian": (
        ...
        "Entry rules for BUYING (ALL must be true):\n"
        ...
        "Exit rules — adapt to the macro trend:\n"
        "- If 1-day SMA(7) > SMA(20) (bullish macro): sell HALF at 1-hour RSI > 70 "
        "(only if position is up > 3% to justify the exit fee), "
        "sell the rest at RSI > 80 or price >= 1-day upper Bollinger Band. "
        "This lets winners run further in uptrends.\n"
        "- If 1-day SMA(7) < SMA(20) (bearish macro): sell ALL at 1-hour RSI > 70. "
        "In bear markets, take profits quickly — rallies are shorter.\n"
        "- FEE-AWARE BREAKEVEN STOP: if position is up > 5% and price drops back toward "
        "entry + 1.5% (enough to cover exit fee), sell ALL. You need the price gain to "
        "exceed round-trip fees (~1.6% with limit entry + market exit) to profit. "
        "A 3% gain is only ~1.4% after fees — protect gains above 5%.\n\n"
        "Risk management:\n"
        ...
        "- Stop-loss: use 1.5x the 1-hour ATR, max 3% from entry\n"
