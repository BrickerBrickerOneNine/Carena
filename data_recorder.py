...
from trading_tools import AccountStore, TAX_RATE
...
class DataRecorder:
    ...
    def record_trade(
        ...
        *,
        ...
        estimated_tax = max(0.0, trade_pnl) * TAX_RATE if trade_pnl is not None else None
        row = TradeRow(
        ...
        for agent_id, account in accounts.items():
            ...
            if not account.positions:
                ...
                for pid, qty in sorted(account.positions.items()):
                    ...
                    market_price = ((float(entry["best_bid"]) + float(entry["best_ask"])) / 2) if entry else 0.0
