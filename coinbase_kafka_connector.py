...
# Event-driven invocation: trigger immediate publish when any product moves this much
PRICE_MOVE_THRESHOLD_PCT = 0.5  # 0.5% move triggers immediate agent invocation
...
class CoinbaseKafkaConnector:
    ...
    def __init__(
        ...
        # Price at last publish per product — for event-driven threshold detection
    ...
    async def _publish_latest(self) -> None:
        ...
        if self._candle_book is not None and self._candle_book.has_data():
            # Build ticker_data dict so indicators can compute spread %
            ticker_data = {
                t.product_id: {"best_bid": t.best_bid, "best_ask": t.best_ask}
                for t in batch
            }
            prompt_parts.append(
        ...
        # Record published prices for threshold detection
        for t in batch:
            try:
    ...
    async def _consume_and_publish(self) -> None:
        ...
        async with websockets.connect(COINBASE_WS_URL) as ws:
            ...
            if self._candle_book is not None:
                # Only fetch candles — live prices come from the WebSocket.
            ...
            try:
                async for raw in ws:
                    ...
                    # Event-driven: immediate publish if price moved significantly
                    try:
                        current_price = float(ticker.price)
                        last_price = self._last_published_prices.get(ticker.product_id)
                        if last_price is not None and last_price > 0:
                            move_pct = abs(current_price - last_price) / last_price * 100
                            if move_pct >= PRICE_MOVE_THRESHOLD_PCT:
                                logger.info(
                                    "Price move threshold triggered for %s: %.2f%% move",
                                    ticker.product_id,
                                    move_pct,
                                )
                                await self._publish_latest()
